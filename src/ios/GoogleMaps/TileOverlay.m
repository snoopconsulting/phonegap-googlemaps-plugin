...
-(void)createTileOverlay:(CDVInvokedUrlCommand *)command
{
  NSDictionary *json = [command.arguments objectAtIndex:1];
    ...
    BOOL geoJson = false;
    @try {
        geoJson = [[json valueForKey:@"geoJSONTile"] boolValue];
    }
    @catch (NSException * e) {
        // Key did not exist
    }
    if ( geoJson ) {
        GeoJSONTileProvider *layer = [GeoJSONTileProvider tileLayer];

        layer.map = self.mapCtrl.map;
        layer.commandDelegate = self.commandDelegate;
        layer.webView = self.webView;
